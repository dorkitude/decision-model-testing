...
func (h *Harness) filterPayload(question, query string, units []Unit) M {
	...
	for i, u := range units {
		...
		if h.C.FilterMode == "categorical" {
			questions[key] = categoricalQuestion(key)
		}
	}
...
func (h *Harness) filter(q Question, s Search) ([]Unit, string, error) {
	...
	for pi, page := range pages {
		...
		for i, u := range page {
			...
			decision, err := filterDecision(h.C.FilterMode, a, h.C.Threshold)
			if err != nil {
				return nil, "", err
			}
			decision["chunk_id"] = u.ID
			decision["page"] = pi
	...
	if e = h.S.put("filters", M{"id": id, "question_id": q.ID, "search_id": s.ID, "original_question": q.Question, "query": s.Query, "threshold": filterThreshold(h.C), "filter_mode": h.C.FilterMode, "decisions": decisions, "kept_ids": ids, "page_request_keys": pageIDs, "pages": len(pages), "elapsed_s": time.Since(start).Seconds()}); e != nil {
		return nil, "", e
	}
	return kept, id, nil
}
...
func (h *Harness) answer(q Question, arm string, initial Search) error {
	...
	// Same initial question-query tool call in both arms; subsequent searches are chosen by Claude.
	...
	for turn := 0; turn < 6; turn++ {
		...
		r, e := h.claudeCall(fmt.Sprintf("answer/%s/turn-%d", id, turn), "answer", payload)
		if e != nil {
			return e
		}
		cs := arr(r["choices"])
		...
		if len(calls) == 0 {
			...
			if str(c["finish_reason"]) != "stop" || answer == "" {
				return fmt.Errorf("incomplete Claude answer for %s: %v", id, c["finish_reason"])
			}
		...
		if choice == "none" {
			return fmt.Errorf("Claude returned tools after budget exhausted")
		}
...
func (h *Harness) runJudges() error {
	...
	return h.parallel(ids, func(id string) error {
		...
		fmt.Printf("judgments %d/%d\n", len(h.S.all("judgments")), 2*h.C.Questions)
		return nil
	})
}
