...
func (h *Harness) freeze() error {
	...
	meta := M{"id": "protocol", "enronqa_cli_commit": string(bytes.TrimSpace(submodule)), "corpus_sha256": hash(corpus), "config": h.C, "questions_sha256": hash(b), "dataset_revision": revision, "prompts_sha256": hash([]byte(answerPrompt + filterPrompt + judgePrompt)), "source_sha256": sourceHash()}
...
func mainErr() error {
	...
	root := &cobra.Command{Use: "jev-search-result-narrower", Short: "Paired EnronQA hybrid-search / Jev-filter experiment", SilenceUsage: true}
	...
	for _, name := range []string{"prepare", "preflight", "run", "report", "status"} {
		...
		root.AddCommand(&cobra.Command{Use: name, RunE: func(cmd *cobra.Command, args []string) error {
			...
			_ = godotenv.Load(filepath.Join(home, ".secrets/keys.env"))
			s, e := newStore(run)
			if e != nil {
				return e
			}
