...
func (h *Harness) report() error {
	...
	if len(answers) != 2*len(qs) || len(judgments) != 4*len(qs) {
	...
	for _, arm := range []string{"baseline", "jev_filtered"} {
		agree := 0
		for _, q := range qs {
			if jmap[q.ID+"/"+arm+"/jev"]["correct"] == jmap[q.ID+"/"+arm+"/deepseek"]["correct"] {
				agree++
			}
		}
		agreements[arm] = M{"agree": agree, "n": len(qs)}
	}
	...
	b.WriteString("## Results\n\nCompleted 100 paired test questions, 200 Kimi K3 answers, and 400 independent judge verdicts (one per answer per judge). No failed or missing cases are excluded.\n\n| Judge | Hybrid + rerank | + Jev filtering | Difference | Paired 95% interval |\n|---|---:|---:|---:|---:|\n")
	for _, judge := range []string{"jev", "deepseek"} {
