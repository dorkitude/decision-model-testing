...
func auditEvidence(run, data, source string) (M, error) {
	...
	for file, key := range map[string]string{"questions.jsonl": "questions_sha256", "units.jsonl": "corpus_sha256"} {
	...
	if len(qs) != int(num(cfg["questions"])) || len(qs) == 0 || len(answers) != len(qs)*2 || len(judgments) != len(qs)*4 {
	...
	for id, f := range filters {
		...
		for page, key := range arr(f["page_request_keys"]) {
			...
			for i, v := range pageResults {
				...
				prob, ok := obj(response[fmt.Sprintf("r%d", i)])["noul"].(float64)
				if !ok || prob < 0 || prob > 1 || d["chunk_id"] != u["chunk_id"] || d["probability_useful"] != prob || d["keep"] != (prob >= num(cfg["filter_threshold"])) || num(d["page"]) != float64(page) {
	...
	for qid, q := range qs {
		for _, arm := range []string{"baseline", "jev_filtered"} {
			...
			for _, judge := range []string{"jev", "deepseek"} {
				...
				var verdict bool
				if judge == "jev" {
					state = obj(obj(req["payload"])["state"])
					choice := str(obj(obj(obj(req["response"])["answers"])["evaluation"])["choice"])
					if choice != "correct" && choice != "incorrect" {
						return nil, fmt.Errorf("invalid Jev verdict")
					}
					verdict = choice == "correct"
				} else {
	...
	for _, req := range requests {
		if req["phase"] != "filter" && req["phase"] != "judge-jev" {
			continue
		}
